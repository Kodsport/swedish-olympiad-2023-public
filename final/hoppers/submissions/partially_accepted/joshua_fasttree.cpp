// Should solve O(rcn), O(n^2) and no big cycles
...
incmap findcollisions(p2 u, vvvp2& redges)
{
    ...
    repe(e, redges[u.first][u.second])
    {
        ...
        repe(k, *r.killist)
        {
            int relativetime = k + r.inc - ancestors.inc;
            if (setcontains(ancestors, relativetime))
            {
                ans[ancestors[relativetime]] = u;
                ancestors.erase(relativetime);
            }
            ancestors.killist->insert(relativetime);
        }
        repe(e, r)
        {
            int t = e.first + r.inc;
            int relativetime = t - ancestors.inc;
            if (setcontains((*ancestors.killist), relativetime))
            {
                ans[e.second] = u;
                continue;
            }
            ...
            {
                ancestors[relativetime] = e.second;
            }
        }
    }
    ancestors.killist = new set<int>();
