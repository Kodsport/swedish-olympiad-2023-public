...
int solve(int r, int c, int k, int n, const vi& cols, vp2& obstacles)
{

    vvi grid(r + 2, vi(c, AIR));

    rep(i, n) grid[obstacles[i].first][obstacles[i].second] = STONE;

    rep(i, k)
    {
        queue<p2> q;
        q.emplace(r + 1, cols[i]);
        ...
        while (q.size())
        {
            ...
            if (p.second < 0 || p.second >= c) continue;
            if (grid[p.first][p.second] > 0) continue; // Air or stone
            grid[p.first][p.second] = WATER;
            if (p.first == 0) continue;

            if (grid[p.first - 1][p.second] == STONE)
            {
    ...
    int ans = 0;
    rep(i, c) ans += grid[0][i] == WATER;
    return ans;
}

int solvesmart(int r, int c, int k, int n, const vi& cols, vp2& obstacles)
{

    set<int> haswater;
    rep(i, k) haswater.insert(cols[i]);
    map<int, set<int>> rows;
    rep(i, n) rows[-obstacles[i].first].insert(obstacles[i].second);
        ...
        while (it != row.second.end())
        {
            ...
            if (setcontains(haswater, *it))
            {
                ...
                while (1)
                {
                    ...
                    if (setcontains(rows, rownr - 1) && setcontains(rows[rownr - 1], *itl - 1))
                    {
                        works = 0;
                        break;
                    }
                    if (setcontains(haswater, *itl)) break;


                    if ((itl == row.second.begin() || *itl - 1 != *prev(itl)))
                    {
                        break;
                    }
                    itl = prev(itl);
                ...
                if (works && *itl - 1 >= 0)
                {
                    haswater.insert(*itl - 1);
                }



                works = 1;
                while (it != prev(row.second.end()) && *it + 1 == *next(it))
                {
                    haswater.erase(*it + 1);
                    if (setcontains(rows, rownr - 1) && setcontains(rows[rownr - 1], *it + 1))
                    {
                        works = 0;
                        break;
                    }
                ...
                {
                    int v;
                    if (it == row.second.end()) v = *prev(it);
                    else v = *it;

                    if (v + 1 < c && !(setcontains(rows, rownr - 1) && setcontains(rows[rownr - 1], v + 1))) haswater.insert(v + 1);
                }
...
int32_t main()
{
    ...
    dread2(int, r, c);
    dread2(int, n, k);
    readvector(int, cols, k);
    vp2 blocks(n);
    rep(i, n)read(blocks[i].first), read(blocks[i].second);

    int anssmart = solvesmart(r, c, k, n, cols, blocks);
    int ansstupid = solve(r, c, k, n, cols, blocks);
    cout << anssmart << " " << ansstupid;
#else
    ...
    rep(t, 1000000)
    {
        ...
        set<int> watercols;
        while (watercols.size() < k) watercols.insert(randint(0, c - 1));

        vp2 obstacles(n);
        rep(i, n) obstacles[i].first = randint(0, r - 1), obstacles[i].second = randint(0, c - 1);

        int anssmart = solvesmart(r, c, k, n, vi(all(watercols)), obstacles);
        int ansstupid = solve(r, c, k, n, vi(all(watercols)), obstacles);
